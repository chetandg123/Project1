...
class SAR(unittest.TestCase):
    ...
    def test_select_district_block_cluster(self):
            ...
            time.sleep(5)
            for x in range(4, len(select_district.options)):
                select_district.select_by_index(x)
                time.sleep(3)
                for y in range(4, len(select_block.options)):
                    ...
                    #     elem = self.driver.find_element_by_xpath(Data.schoolcount).text
                    #     res = re.sub("\D", "", elem)
                    #     if int(len(list) - 1) != int(res):
                    #         print(x, " ", y, " ", z)
                    #         self.assertEqual(int(len(list) - 1), int(res), "no of dots is not equal to no of schools")
                    #     self.assertEqual(len(list)-1,int(res),msg="mismatch of dots found!")
                    #     time.sleep(3)
